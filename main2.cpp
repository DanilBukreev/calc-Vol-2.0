#
include < iostream > #include < string > #include < locale > #include < stdlib.h > #include < cmath >
 using namespace std;
int ADD(int a, int b) {
 return a + b;
}
  ...
  do {
   do {
    ...
    if ((operation != '+') && (operation != '-') && (operation != '*' &&
      ...
      (operation != '!') && (operation != '&') && (operation != '|') &&
      (operation != '<') && (operation != '>')); cerr << "Некорректные данные!" << endl;
    }
    while ((operation != '+') && (operation != '-') && (operation != '*' &&
     ...
     if ((operation != '+') || (operation != '-') || (operation != '*' ||
       ...
       do {
        ...
        cin >> ch1;
        if (!(fail = cin.good())) cout << "error" << endl;
        cin.clear();
       ...
       do {
        ...
        cin >> ch2;
        if (!(fail = cin.good())) cerr << "error" << endl;
        cin.clear();
      ...
      cout << "Продолжить работу? (y/n)" << endl; cin >> s;
     }
     while (s == 'y');
     return 0;
    }
